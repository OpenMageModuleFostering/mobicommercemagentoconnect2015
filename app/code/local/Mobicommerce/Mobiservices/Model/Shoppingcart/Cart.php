...
class Mobicommerce_Mobiservices_Model_Shoppingcart_Cart extends Mobicommerce_Mobiservices_Model_Abstract
{
    ...
    public function getCartInfo(){
        ...
        if($sessionCustomer->isLoggedIn()) {
            ...
            $userinfo = Mage::getModel('mobiservices/user')->_getCustomerProfileData($_customer);
        }
        $cart_array['userinfo'] = $userinfo;
        $cart_array['wishlist'] = Mage::getModel('mobiservices/wishlist_wishlist')->getWishlistInfo();
        return $cart_array;
    }
            ...
            if(in_array($method->getCode(), array(
                ...
                foreach($installment_key_pair as $installment_key => $installment_pair){
                    if($method->getConfigData($installment_pair['keycode'])){
                        ...
                        $installment_key_pair[$installment_key]['options'] = Mage::getModel('mobiservices/custom')->_processPayuapiInstallmentOptionsString($installment_key_pair[$installment_key]['options_str'], $grandTotal, $interest_type = 'simple', $force_yearly_interest = true);
    ...
    public function savePaymentMethod($data){
        ...
        if(!$paymentStatus){
            ...
            $info = Mage::getModel('mobiservices/shoppingcart_cart')->errorStatus($error);
            return $info;
        }
        $info = $this->successStatus();
        $info['data']['agreements'] = Mage::getModel('mobiservices/config')->_getAgreements();
        $info['data']['cart_details'] = $this->getCartInfo();
        return $info;
    }
    ...
    public function validateOrder($data){
        ...
        //return array('test' => 2, 'cart_details' => $this->getCartInfo());
    ...
    public function saveOrder($data){
        ...
        $_result['data']['order_data'] = Mage::getModel('mobiservices/user')->_getOrderHistory();
        return $_result;
    }
